...
TEST_CASE("ip case 0", "[ip]") {
    ...
    auto res = solver.Solve(8900L, 1370390000L);
    REQUIRE(res == OPTIMAL);
...
TEST_CASE("ip case 1 hit avg price", "[ip]") {
    ...
    REQUIRE(solver.to_string_or_constraint() == cons_str);
    auto res = solver.Solve(5000L, 1000000L);
    REQUIRE(res == OPTIMAL);
...
TEST_CASE("ip case 1", "[ip]") {
    ...
    REQUIRE(solver.to_string_or_objective() == obj_str);
    REQUIRE(solver.to_string_or_constraint() == cons_str);
    auto res = solver.Solve(13500L, 3736620000L);
...
TEST_CASE("ip case 2", "[ip]") {
    ...
    REQUIRE(solver.to_string_or_constraint() == cons_str);
    auto res = solver.Solve(60900L, 4421300000L);
    REQUIRE(res == OPTIMAL);
...
TEST_CASE("ip case 3", "[ip]") {
    ...
    auto res = solver.Solve(101000L, 11350960000L);
    REQUIRE(res == OPTIMAL);
